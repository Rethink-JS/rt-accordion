...
(() => {
  ...
  (function() {
    ...
    Accordion.prototype.computeDurationMs = function(currentPx, targetPx, opening) {
      var delta = Math.abs(targetPx - currentPx);
      var speed = opening ? SPEED_OPEN_PX_PER_S : SPEED_CLOSE_PX_PER_S;
      var ms = delta / Math.max(1, speed) * 1e3;
    ...
    Accordion.prototype.setItemOpen = function(item, triggerEl, contentEl, open, immediate) {
      if (!item) return;
      var current = item.offsetHeight || 0;
      var target = open ? this.computeExpandedHeight(triggerEl, contentEl) : triggerEl ? triggerEl.offsetHeight : 0;
